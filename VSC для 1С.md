## Трюки
### Как выгруженную из отладчика строку ТЗ превратить в описание ТЗ
Ищем: `(\S+)(.*)\s(\S+)$`
Меняем: `ТЗ.Колонки.Добавить("$1", Новый ОписаниеТипов("$3")); // $2`
Описание: идем с двух сторон, слева: две косых, непробельное слово. Справа: От конца строки непробельное слово (это тип), пробельный символ. Серединку выводим как комментарий.

Получаем из `EmailЛичный	""	Строка` код для создания `ТЗ.Колонки.Добавить("EmailЛичный", "Строка"); //    ""`

Потом 
Ищем: ``(\S+)(.*)\s(\S+)$``
Меняем `НоваяСтрока.$1 = $2; // "$3"`
и будет заполнение строки. с НБСП в числах, незакавыченными названиями справочников и хз что в перечислениях. NULL тоже не проставляется. Но если это все зарешать боюсь регулярка станет на лист. (да, даты тоже кривые)
